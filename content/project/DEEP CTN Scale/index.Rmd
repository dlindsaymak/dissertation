...
#merging data and extreme item kurtosis and skewedness scores
extremetable <- 
  extremetable %>% 
  arrange(skew) %>% 
...
```{r Correlation plots within, echo=FALSE, out.width="100%"}
# Deep
deep_items <-
  items %>% 
...
```{r factor four fit indices, echo=FALSE}
...
```{r heirarchial fit indices, echo=FALSE}
...
Fit_table <- 
  ...
  dplyr::select(Model, df, RMSEA, RMSR, TLI, CFI, AIC) 
...
fit_red <- tibble(chi_dfr, rmsea_cir, srmrr, tlir)
...
```{r reduced fit table, echo=FALSE}
...
fac_plot <- 
  ...
  ggplot(aes(X1, X2, fill = R, label = label)) +
    ...
    geom_text(color = ifelse(fac_d$R > 0.60, "black", "white"), size = 5) +
    theme_bw() +
    coord_equal() +
     theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
alpha_emo <- ltm::cronbach.alpha(reduced[,11:13])
alpha_emo <- round(alpha_emo$alpha,2)
alpha_pres <- ltm::cronbach.alpha(reduced[,14:18])
...
Environmental Identity Scale revised (EID-R - Clayton et al., 2021)(cronbach's alpha = `r round(eidr_alpha$alpha,2)`).
...
validation_plot <- 
  ggplot(d, aes(X1, X2, fill = R, label = label)) +
    ...
    geom_text(color = ifelse(d$R > 0.56, "black", "white"), size = 4) +
    theme_bw() +
    coord_equal() +
     theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
-   Hypothesis 2A is supported as the Deep CTN scale facet correlated
    strongly with the CNS (r = `r round(d[10,3],2)`), while the other
    three facets only correlated moderately (experiential r =
    `r round(d[14,3],2)`, emotional r = `r round(d[17,3],2)`, presence r
    = `r round(d[20,3],2)`).

-   Hypothesis 2B is partially supported. While the experiential facet
    did correlate strongly with the EID-R (r = `r round(d[15,3],2)`),
    the emotional facet only correlated moderately (r =
    `r round(d[18,3],2)`). Additionally, the deep and presence facets
    also correlated moderately (r = `r round(d[11,3],2)` and r =
    `r round(d[20,3],2)` respectively).
...
`r round(peb_alpha$alpha,2)`). A zero-order correlation confirmed moderate correlations between the factors and PEB. However, we also predicted some variation of correlations between the facets such that some are stronger predictors than others. This was not supported as correlations ranged between r = 0.43 and r = 0.47 (See Figure 8.)

```{r zero order correlation PEB, include=FALSE, echo=FALSE}

peb_cor <-
  val_data %>% 
  dplyr::select(RPEBS_z, DeepCTN_z, ExpCTN_z, EmoCTN_z, PresCTN_z) %>% 
  as.matrix() %>% 
  rcorr(type = "spearman")

peb_row = rownames(peb_cor$r)
peb_m = t(combn(peb_row, 2))
peb_d = cbind(data.frame(peb_m), R = peb_cor$r[peb_m], P = peb_cor$P[peb_m])
peb_d$label = round(peb_d$R, 2)
peb_d$label[peb_d$P < 0.001] = paste0(peb_d$label[peb_d$P < 0.001], "\n***")
peb_d$X1 = factor(peb_d$X1, peb_row)
peb_d$X2 = factor(peb_d$X2, rev(peb_row))
graphics.off()
peb_plot <- 
  peb_d %>% 
  ggplot(aes(X1, X2, fill = R, label = label)) +
    geom_tile(color = "white") +
    scale_fill_viridis_c(begin = 0, end = 1) +
    geom_text(color = ifelse(peb_d$R > 0.60, "black", "white"), size = 3) +
    theme_bw() +
    coord_equal() +
     theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r zero order correlation PEB plot, echo=FALSE, include=TRUE, out.width="100%"}
peb_plot
```
...
wb_cor <-
  val_data %>% 
  dplyr::select(WB, DeepCTN_z, ExpCTN_z, EmoCTN_z, PresCTN_z) %>% 
  as.matrix() %>% 
  rcorr(type = "spearman")

wb_row = rownames(wb_cor$r)
wb_m = t(combn(wb_row, 2))
wb_d = cbind(data.frame(wb_m), R = wb_cor$r[wb_m], P = wb_cor$P[wb_m])
wb_d$label = round(wb_d$R, 2)
wb_d$label[wb_d$P < 0.001] = paste0(wb_d$label[wb_d$P < 0.001], "\n***")
wb_d$X1 = factor(wb_d$X1, wb_row)
wb_d$X2 = factor(wb_d$X2, rev(wb_row))
graphics.off()
wb_plot <- 
  wb_d %>% 
  ggplot(aes(X1, X2, fill = R, label = label)) +
    geom_tile(color = "white") +
    scale_fill_viridis_c(begin = 0, end = 1) +
    geom_text(color = ifelse(wb_d$R > 0.60, "black", "white"), size = 3) +
    theme_bw() +
    coord_equal() +
     theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r zero order correlation WB plot, echo=FALSE, include=TRUE, out.width="100%"}
wb_plot
```
...
#gender and age need to be included    
allvarscordata <- 
     val_data %>% 
         dplyr::select(CTN_z,
                 DeepCTN_z,
...
### Hypothesis 5.A - PEB 
```{r Robustness dimsonly, echo=FALSE, include=FALSE, warning=FALSE}
...
robust_pvalues <- tibble(round(robust_anova_peb$`Pr(>F)`,2),
                         .name_repair = "universal") #p values
vars <- tibble(c("deep_ctn_z", "exp_ctn_z", "emo_ctn_z", "presc_ctn_z", "PI_connect_Z"))
...
R2_incremental_peb_robust <- 
     bind_cols(vars, robust_rvalues, robust_pvalues[-6,])%>% 
     dplyr::rename("variable" = `c("deep_ctn_z", "exp_ctn_z", "emo_ctn_z", "presc_ctn_z", "PI_connect_Z")`, 
            "Partial R2" = `round.robust_anova_peb..Pr..F....2.`, 
...
```{r robust anova table, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
#Compare models
...
```{r Robustness Existing measures, echo=TRUE, warning=FALSE}
...
```{r robust PEB output, warning=FALSE, include=FALSE}
...
robust_pvalues <- tibble(round(robust_anova_wb$`Pr(>F)`,2),
                         .name_repair = "universal") #p values
vars <- tibble(c("deep_ctn_z", "exp_ctn_z", "emo_ctn_z", "presc_ctn_z", "age"))
...
R2_incremental_wb_robust <- 
     bind_cols(vars, robust_rvalues, robust_pvalues[-6,])%>% 
     rename("variable" = `c("deep_ctn_z", "exp_ctn_z", "emo_ctn_z", "presc_ctn_z", "age")`, 
...
Adding age as an additional covariate improved the fit of the model above the variables in the incremental validity model (F = `r robust_wb_f`(`r robust_wb_df`), p = `r robust_wb_p`).
