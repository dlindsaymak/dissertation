...
deep_plot <- 
  ...
  ggplot(aes(X1, X2, fill = R, label = label)) +
    ...
    geom_text(color = ifelse(deep_d$R < 0.60, "black", "white"), size = 1) +
    theme_bw() +
    coord_equal() +
     theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
exp_plot <- 
  ...
  ggplot(aes(X1, X2, fill = R, label = label)) +
    geom_tile(color = "white") +
    scale_fill_viridis_c(begin = 1, end = 0) +
    geom_text(color = ifelse(exp_d$R < 0.60, "black", "white"), size = 1) +
    theme_bw() +
    coord_equal() +
     theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
pres_plot <- 
  ...
  ggplot(aes(X1, X2, fill = R, label = label)) +
    geom_tile(color = "white") +
    scale_fill_viridis_c(begin = 1, end = 0) +
    geom_text(color = ifelse(pres_d$R < 0.60, "black", "white"), size = 1) +
    theme_bw() +
    coord_equal() +
     theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
fac_cor <-
  val_data %>% 
  mutate(Deep = (CTN_spirit_1 + CTN_spirit_2 + CTN_spirit_3 + CTN_spirit_7 + CTN_spirit_8)/5,
         Experience = (CTN_exp_1+ CTN_exp_2+ CTN_exp_3+ CTN_exp_4+ CTN_exp_5)/5,
         Emotional = (CTN_emo_3+ CTN_emo_4+ CTN_emo_6)/3,
...
```{r linear regression with only dimensions predicting PEB, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r dimonly output, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r linear regression with only dimensions predicting WB, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r dimonly output WB, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
However, adding the additional covariate (interconnected primal beliefs) did not improve the fit of the model above the variables in the incremental validity model (F = `r robust_peb_f`(`r robust_peb_df`), p = `r robust_peb_p`).
