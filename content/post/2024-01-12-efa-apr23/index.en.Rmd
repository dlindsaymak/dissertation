...
p2_prolific$gender <- 
  factor(p2_prolific$gender, levels = c("man", "woman", "trans man", "trans woman", "non-binary", "other", "no_answer"))
...
m_pol <- paste(round(mean(p2_prolific$politics_1, na.rm = T),2))
sd_pol <- paste(round(sd(p2_prolific$politics_1, na.rm = T),2))
...
p2_prolific %>% 
     ...
     summarise(n = length(id)) %>% 
  mutate(`Ethnicity simplified` = 
           if_else(`Ethnicity simplified` == "DATA_EXPIRED", "Unavailable", paste(`Ethnicity simplified`))) %>% 
  mutate(`Ethnicity simplified` = factor(`Ethnicity simplified`, levels = c("White", "Black", "Asian", "Mixed", "Other", "Unavailable"))) %>% 
     flextable() %>% 
  autofit() #%>% 
...
```{r box1, echo=FALSE}
boxplot(factor_order_prolific[,c(1:8)],
        las = 2)
```

```{r box2, echo=FALSE}
boxplot(factor_order_prolific[,c(9:17)],
        las = 2)
```

```{r box3, echo=FALSE}
boxplot(factor_order_prolific[,c(18:26)],
        las = 2)
```


```{r box4, echo=FALSE}
boxplot(factor_order_prolific[,c(27:34)],
        las = 2)
```

```{r box5, echo=FALSE}
boxplot(factor_order_prolific[,c(34:38)],
        las = 2)
```
...
eigens_prolific <-
  eigen(cor(factor_order_prolific))
...
fit_6_prolific <- 
  fa(factor_order_prolific, 6, rotate="promax", fm= "pa") %>% 
  fa.sort
...
fit_5_prolific <- 
  fa(factor_order_prolific, 5, rotate="promax", fm= "pa") %>% 
  fa.sort
...
#principal axis (for non-normal data)
fit_4_prolific <- 
  fa(factor_order_prolific, 4, rotate="promax", fm= "pa") %>% 
  fa.sort
