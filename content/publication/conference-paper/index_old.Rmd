...
items %>% 
  dply::select(`Original Scale`, `Original Item`, Reworded, Notes) %>% 
  kableExtra::kbl("html") %>% 
...
parallel_1 <- 
  psyc::fa.parallel(efa_1_items, fa = "fa")
...
parallel_2 <- 
  psyc::fa.parallel(efa_2_items, fa = "fa")
...
parallel_3 <- 
  psyc::fa.parallel(efa_3_items, fa = "fa")
...
4.  P**resence within nature** - Engaging mindfully and consciously with nature

```{r fit indices table, message=FALSE, warning=FALSE, include=FALSE}
#Sample 1
fit_6_efa1 <- 
  psyc::fa(efa_1_items, 6, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
fit_5_efa1 <- 
  psyc::fa(efa_1_items, 5, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
fit_4_efa1 <- 
...
fit_4_red_efa1 <- 
  psyc::fa(efa_1_items_reduced, 4, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
...
#Sample 2
fit_6_efa2 <- 
  psyc::fa(efa_2_items, 6, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
fit_5_efa2 <- 
  psyc::fa(efa_2_items, 5, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
fit_4_efa2 <- 
  psyc::fa(efa_2_items, 4, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
efa_2_items_reduced <-
  efa_2_items %>% 
  dplyr::select(-NEW_7,
         ...
         -DISPO_23)
fit_4_red_efa2 <- 
  psyc::fa(efa_2_items_reduced, 4, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
...
#Sample 3
fit_6_efa3 <- 
  psyc::fa(efa_3_items, 6, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
fit_5_efa3 <- 
  psyc::fa(efa_3_items, 5, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
fit_4_efa3 <- 
  psyc::fa(efa_3_items, 4, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
efa_3_items_reduced <-
  efa_3_items %>% 
  dplyr::select(-AIMES_8,
        ...
        -DISPO_20)
fit_4_red_efa3 <- 
  psyc::fa(efa_3_items_reduced, 4, rotate="promax", fm= "pa") %>% 
  psyc::fa.sort
...
Fit_table %>% 
  ...
  tidyr::mutate(chi = round(as.numeric(chi),3),
...
set_flextable_defaults(na_str = NULL, scroll = list())
...
```{r CFA Kurtosis Figure, message=FALSE, warning=FALSE, include=FALSE}

kurtosis <- as.data.frame(round(sort(kurtosis(cfa_items),
     decreasing=T),2))

kurtosis_items <- 
kurtosis %>% 
  dplyr::rename("kurtosis" = "round(sort(kurtosis(cfa_items), decreasing = T), 2)") %>% 
...
presence =~
  ...
  DEEPCTN_m_5 +
  DEEPCTN_m_6'
fit_model_flat_4 <- lavvan::cfa(model_flat_4, data=cfa_items,
                        std.lv=F, #set to T to get Std.Err!!!
